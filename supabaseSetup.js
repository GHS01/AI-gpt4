...
async function setupDatabase() {
  ...
  try {
    // Verificar si el cliente de Supabase está inicializado correctamente
    if (!supabase) {
      console.error('Error: Cliente de Supabase no inicializado. Saltando configuración de base de datos.');
      return;
    }
    ...
    if (error) {
      console.warn('Error al verificar conexión con Supabase:', error.message);
      console.warn('Es posible que la base de datos no esté disponible o que las credenciales sean incorrectas.');
      console.warn('Continuando con la inicialización, pero la funcionalidad podría estar limitada.');
      return;
    }

    console.log('Conexión con Supabase establecida correctamente, continuando con la configuración...');
