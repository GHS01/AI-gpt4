...
console.log('SUPABASE_URL está configurado:', !!process.env.SUPABASE_URL);
console.log('SUPABASE_ANON_KEY está configurado:', !!process.env.SUPABASE_ANON_KEY);
console.log('ADMIN_ACCESS_CODE está configurado:', !!process.env.ADMIN_ACCESS_CODE);
...
app.get('/api/config', (req, res) => {
  try {
    ...
    res.json({
      adminAccessCode: process.env.ADMIN_ACCESS_CODE || '997796929'
    });
  } catch (error) {
...
if (process.env.NODE_ENV !== 'production') {
  ...
  // Para entorno de producción (Vercel)
  // Inicializar la base de datos
  initializeDatabase().then(() => {
    console.log('Base de datos inicializada en entorno de producción');
  }).catch(err => {
    console.error('Error al inicializar la base de datos en producción:', err);
  });
}
